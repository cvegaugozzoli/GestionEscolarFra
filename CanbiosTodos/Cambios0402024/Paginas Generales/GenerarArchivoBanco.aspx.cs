...
protected void btnAceptar_Click(object sender, EventArgs e)
    {
        ...
        if (dt1.Rows.Count > 0)
        {
            ...
            foreach (DataRow row in dt1.Rows)
            {
                ...
                intMensual = Convert.ToDecimal(row["ConInteresMensual"].ToString());
                ...
                //if ((pdni == "47502348")) // || (pdni == "49349537")
                //{
                //    pok = "1";
                //}
                ...
                if (dt2.Rows.Count > 0)
                {
                    foreach (DataRow row2 in dt2.Rows)
                    {
                        ...
                        {
                            ImpMes[C] = Convert.ToDecimal(row["ImpConBeca"].ToString()) + Convert.ToDecimal(row2["coiValorInteres"].ToString());
                        } else
                        {
                            ImpMes[C] = Convert.ToDecimal(row["icoImporte"].ToString()) + Convert.ToDecimal(row2["coiValorInteres"].ToString());
                        }
                        C += 1;
                    }
                }
                ...
                if (row["conTieneVtoAbierto"].ToString() == "1")
                {
                    if (AplicaInteresAbierto[C-1] == "0")
                    {
                        if (AplicaBeca[C-1] == "1")
                        {
                            ...
                            ImpMes[C] = Convert.ToDecimal(row["ImpConBeca"].ToString());
                        }
                        else
                        {
                            Recargo = Convert.ToSingle(row["icoImporte"].ToString());
                            ImpMes[C] = Convert.ToDecimal(row["icoImporte"].ToString());
                        }
                    }
                    else
                    {
                        Recargo = Convert.ToSingle(row["icoImporte"].ToString()) + Convert.ToSingle(row["conRecargoVtoAbierto"].ToString());
                        ImpMes[C] = Convert.ToDecimal(row["icoImporte"].ToString()) + Convert.ToDecimal(row["conRecargoVtoAbierto"].ToString());
                    }
                } else
                {
                    ...
                    if (intMensual > 0 && Convert.ToDateTime(txtDesde.Text.ToString()) > Convert.ToDateTime(Vtos[2].ToString()) && CantVtos == 2)
                    {
                        ...
                        ImpMes[2] = Decimal.Round((((ImpMes[1] * intMensual)/100) * MultiplicadorCuotas)) + ImpMes[1];
                ...
                barra = barra + DiferenciaImporte3;   // Format(((ImpsMes(3) - CSng(ImpsMes(1))) * 10), "0000")
