...
public partial class ConceptosConsulta : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!Page.IsPostBack)
            {
                ...
                #region PageIndex
                int PageIndex = 0;
                if (this.Session["ConceptosConsulta.PageIndex"] == null)
                {
                    Session.Add("ConceptosConsulta.PageIndex", 0);
                }
                else
                {
                    PageIndex = Convert.ToInt32(Session["ConceptosConsulta.PageIndex"]);
                }
                #endregion
...
"</div>";
    ...
    protected void btnNuevo_Click(object sender, EventArgs e)
    {
	    try
	    {
            Response.Redirect("ConceptosRegistracion.aspx?Id=0", false);
	    }
	    catch (Exception oError)
	    {
            lblMensajeError.Text = @"<div class=""alert alert-danger alert-dismissable"">
...
"</div>";
    ...
    protected void btnExportarAExcel_Click(object sender, EventArgs e)
    {
        insId = Convert.ToInt32(Session["_Institucion"]);
        dt = new DataTable();
        dt = ocnConceptos.ObtenerTodoBuscarxNombre(Nombre.Text.Trim(), insId);
        string ArchivoNombre = "ConceptosConsulta_" + DateTime.Now.Day.ToString() + "-" + DateTime.Now.Month.ToString() + "-" + DateTime.Now.Year.ToString() + ".xls";
    ...
    private void GrillaCargar(int PageIndex)
    {
        try
        {
            Session["ConceptosConsulta.PageIndex"] = PageIndex;
            ...
            {
                dt = new DataTable();
                dt = ocnConceptos.ObtenerTodoBuscarxNombrexAnio(insId,Nombre.Text.Trim(),  txtAnio.Text);
    ...
    protected void Grilla_RowCommand(object sender, GridViewCommandEventArgs e)
    {
	    try
	    {
		    if (e.CommandName != "Sort" && e.CommandName != "Page" && e.CommandName != "")
		    {
			    ...
			    {
				    //ocnConceptos.Eliminar(Convert.ToInt32(Id));
				    this.GrillaCargar(this.Grilla.PageIndex);
			    }

			    if (e.CommandName == "Copiar")
			    {
				    ocnConceptos = new GESTIONESCOLAR.Negocio.Conceptos(Convert.ToInt32(Id));
				    //ocnConceptos.Copiar();
				    this.GrillaCargar(this.Grilla.PageIndex);
			    }
			    ...
			    {
				    Response.Redirect("ConceptosRegistracion.aspx?Id=" + Id + "&Ver=1", false);
			    }			
		    }
	    }
	    catch (Exception oError)
...
"</div>";
    ...
    protected void Grilla_PageIndexChanging(object sender, GridViewPageEventArgs e)
    {
	    try
	    {
            if (Session["ConceptosConsulta.PageIndex"] != null)
            {
                Session["ConceptosConsulta.PageIndex"] = e.NewPageIndex;
            }
            else
            {
                Session.Add("ConceptosConsulta.PageIndex", e.NewPageIndex);
            }
