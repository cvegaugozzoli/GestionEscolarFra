    ...
    private void ImprimirChequera()
    {
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            foreach (DataRow row in dt.Rows)// por cada fila..
            {
                ...
                BarcodeQRCode qrcode = new BarcodeQRCode(Convert.ToString(row["BARRA"].ToString()), 1, 1, null);
                iTextSharp.text.Image barCode = qrcode.GetImage();
                barCode.SetAbsolutePosition(10, 500);
                barCode.ScalePercent(200);
                doc.Add(barCode);
