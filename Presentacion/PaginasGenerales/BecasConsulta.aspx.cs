...
public partial class BecasConsulta : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!Page.IsPostBack)
            {
                this.Master.TituloDelFormulario = " Becas - Consulta";

                //if (this.Session["_Autenticado"] == null) Response.Redirect("Login.aspx", true);

                #region PageIndex
                int PageIndex = 0;
                if (this.Session["BecasConsulta.PageIndex"] == null)
                {
                    Session.Add("BecasConsulta.PageIndex", 0);
                }
                else
                {
                    PageIndex = Convert.ToInt32(Session["BecasConsulta.PageIndex"]);
                }
                #endregion

                #region Variables de sesion para filtros
//if (Session["BecasConsulta.Nombre"] != null) { Becas.Text = Session["BecasConsulta.Nombre"].ToString(); } else { Session.Add("BecasConsulta.Nombre", Nombre.Text.Trim()); }
                #endregion
...
"</div>";
    ...
    protected void btnNuevo_Click(object sender, EventArgs e)
    {
	    try
	    {
            Response.Redirect("BecasRegistracion.aspx?Id=0", false);
	    }
	    catch (Exception oError)
	    {
            lblMensajeError.Text = @"<div class=""alert alert-danger alert-dismissable"">
...
"</div>";
    ...
    {
        dt = new DataTable();
        dt = ocnBecas.ObtenerTodoBuscarxNombre(Nombre.Text.Trim());
        string ArchivoNombre = "BecasConsulta_" + DateTime.Now.Day.ToString() + "-" + DateTime.Now.Month.ToString() + "-" + DateTime.Now.Year.ToString() + ".xls";
    ...
    private void GrillaCargar(int PageIndex)
    {
        try
        {
            Session["BecasConsulta.PageIndex"] = PageIndex;
    ...
    protected void Grilla_RowCommand(object sender, GridViewCommandEventArgs e)
    {
	    try
	    {
		    if (e.CommandName != "Sort" && e.CommandName != "Page" && e.CommandName != "")
		    {
			    ...
			    {
				    ocnBecas = new GESTIONESCOLAR.Negocio.Becas(Convert.ToInt32(Id));
				    //ocnBecas.Copiar();
				    this.GrillaCargar(this.Grilla.PageIndex);
			    }
			    ...
			    {
				    Response.Redirect("BecasRegistracion.aspx?Id=" + Id + "&Ver=1", false);
			    }			
		    }
	    }
	    catch (Exception oError)
...
"</div>";
    ...
    protected void Grilla_PageIndexChanging(object sender, GridViewPageEventArgs e)
    {
	    try
	    {
            if (Session["BecasConsulta.PageIndex"] != null)
            {
                Session["BecasConsulta.PageIndex"] = e.NewPageIndex;
            }
            else
            {
                Session.Add("BecasConsulta.PageIndex", e.NewPageIndex);
            }
