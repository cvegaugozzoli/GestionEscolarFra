...
public partial class IngresoPagos : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!Page.IsPostBack)
            {
                ...
                int PageIndex = 0;
                if (this.Session["IngresoPagos.PageIndex"] == null)
                {
                    Session.Add("IngresoPagos.PageIndex", 0);
                }
                else
                {
                    PageIndex = Convert.ToInt32(Session["IngresoPagos.PageIndex"]);
                }
            }
