...
public partial class RegistracionCalificacionesConsulta : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!Page.IsPostBack)
            {
                Master.TituloDelFormulario = " Registracion Calificaciones - Consulta";

                //if (this.Session["_Autenticado"] == null) Response.Redirect("Login.aspx", true);

                #region PageIndex
                int PageIndex = 0;
                if (this.Session["RegistracionCalificacionesConsulta.PageIndex"] == null)
                {
                    Session.Add("RegistracionCalificacionesConsulta.PageIndex", 0);
                }
                else
                {
                    PageIndex = Convert.ToInt32(Session["RegistracionCalificacionesConsulta.PageIndex"]);
                }
                #endregion
                ...
                if (Request.QueryString["Id"] != null)
                {
                    ...
                    icuId.Text = Id.ToString();
                    DataTable dt = new DataTable();
                    dt = ocnInscripcionCursado.ObtenerUno(Id);
                    if (dt.Rows.Count > 0)
                    {
...
"</div>";
    ...
    protected void btnCancelar_Click(object sender, EventArgs e)
    {
        try
        {
            Response.Redirect("InscripcionCursadoConsulta.aspx?Id=" + icuId.Text, true);
        }
        catch (Exception oError)
        {
            lblMensajeError.Text = @"<div class=""alert alert-danger alert-dismissable"">
...
"</div>";
    ...
    {
        dt = new DataTable();
        dt = ocnRegistracionCalificaciones.ObtenerTodo();
        string ArchivoNombre = "RegistracionCalificacionesConsulta_" + DateTime.Now.Day.ToString() + "-" + DateTime.Now.Month.ToString() + "-" + DateTime.Now.Year.ToString() + ".xls";
    ...
    private void GrillaCargar(int PageIndex)
    {
        try
        {
            Session["RegistracionCalificacionesConsulta.PageIndex"] = PageIndex;
            ...
            if (Request.QueryString["Id"] != null)
            {
                IdIC = Convert.ToInt32(Request.QueryString["Id"]);
                dt = new DataTable();
                dt = ocnRegistracionCalificaciones.ObtenerTodoporInscripcionCursado(IdIC);
    ...
    protected void Grilla_RowCommand(object sender, GridViewCommandEventArgs e)
    {
	    try
	    {
		    if (e.CommandName != "Sort" && e.CommandName != "Page" && e.CommandName != "")
		    {
			    ...
			    {
				    ocnRegistracionCalificaciones = new GESTIONESCOLAR.Negocio.RegistracionCalificaciones(Convert.ToInt32(Id));
			    ...
			    {
				    Response.Redirect("RegistracionCalificacionesRegistracion.aspx?Id=" + Id + "&Ver=1", false);
			    }			
		    }
	    }
	    catch (Exception oError)
...
"</div>";
    ...
    protected void Grilla_PageIndexChanging(object sender, GridViewPageEventArgs e)
    {
	    try
	    {
            if (Session["RegistracionCalificacionesConsulta.PageIndex"] != null)
            {
                Session["RegistracionCalificacionesConsulta.PageIndex"] = e.NewPageIndex;
            }
            else
            {
                Session.Add("RegistracionCalificacionesConsulta.PageIndex", e.NewPageIndex);
            }
...
"</div>";
    ...
    protected void btnEliminarAceptar_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            ocnRegistracionCalificaciones.Eliminar(Id);
