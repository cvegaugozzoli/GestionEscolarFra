...
public partial class DocentesConsulta : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!Page.IsPostBack)
            {
                this.Master.TituloDelFormulario = " Docentes - Consulta";

                //if (this.Session["_Autenticado"] == null) Response.Redirect("Login.aspx", true);

                #region PageIndex
                int PageIndex = 0;
                if (this.Session["DocentesConsulta.PageIndex"] == null)
                {
                    Session.Add("DocentesConsulta.PageIndex", 0);
                }
                else
                {
                    PageIndex = Convert.ToInt32(Session["DocentesConsulta.PageIndex"]);
                }
                #endregion
...
"</div>";
    ...
    protected void btnNuevo_Click(object sender, EventArgs e)
    {
	    try
	    {
            Response.Redirect("DocentesRegistracion.aspx?Id=0", false);
	    }
	    catch (Exception oError)
	    {
            lblMensajeError.Text = @"<div class=""alert alert-danger alert-dismissable"">
...
"</div>";
    ...
    private void GrillaCargar(int PageIndex)
    {
        try
        {
            Session["DocentesConsulta.PageIndex"] = PageIndex;
...
//[VariablesDeSesionParaFiltros1]
            ...
            dt = new DataTable();
                      dt = ocnDocentes.ObtenerUnoxNombre(Nombre.Text.Trim());
...
"</div>";
    ...
    protected void Grilla_PageIndexChanging(object sender, GridViewPageEventArgs e)
    {
	    try
	    {
            if (Session["DocentesConsulta.PageIndex"] != null)
            {
                Session["DocentesConsulta.PageIndex"] = e.NewPageIndex;
            }
            else
            {
                Session.Add("DocentesConsulta.PageIndex", e.NewPageIndex);
            }
