...
public partial class BancosConsulta : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!Page.IsPostBack)
            {
                this.Master.TituloDelFormulario = " Bancos - Consulta";

                //if (this.Session["_Autenticado"] == null) Response.Redirect("Login.aspx", true);

                #region PageIndex
                int PageIndex = 0;
                if (this.Session["BancosConsulta.PageIndex"] == null)
                {
                    Session.Add("BancosConsulta.PageIndex", 0);
                }
                else
                {
                    PageIndex = Convert.ToInt32(Session["BancosConsulta.PageIndex"]);
                }
                #endregion
...
"</div>";
    ...
    protected void btnNuevo_Click(object sender, EventArgs e)
    {
	    try
	    {
            Response.Redirect("BancosRegistracion.aspx?Id=0", false);
	    }
	    catch (Exception oError)
	    {
            lblMensajeError.Text = @"<div class=""alert alert-danger alert-dismissable"">
...
"</div>";
    ...
    private void GrillaCargar(int PageIndex)
    {
        try
        {
            Session["BancosConsulta.PageIndex"] = PageIndex;
...
"</div>";
    ...
    protected void Grilla_PageIndexChanging(object sender, GridViewPageEventArgs e)
    {
	    try
	    {
            if (Session["BancosConsulta.PageIndex"] != null)
            {
                Session["BancosConsulta.PageIndex"] = e.NewPageIndex;
            }
            else
            {
                Session.Add("BancosConsulta.PageIndex", e.NewPageIndex);
            }
