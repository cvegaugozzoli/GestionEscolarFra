    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!Page.IsPostBack)
            {
                ...
                if (this.Session["CursoListadoRegCalifSec.PageIndex"] == null)
                {
                    Session.Add("CursoListadoRegCalifSec.PageIndex", 0);
                }
                else
                {
                    PageIndex = Convert.ToInt32(Session["CursoListadoRegCalifSec.PageIndex"]);
                }
                #endregion
...
"</div>";
    ...
    protected void btnExportarAExcel_Click(object sender, EventArgs e)
    {
        ...
        string ArchivoNombre = "CargaCalifxEspCSec" + DateTime.Now.Day.ToString() + "-" + DateTime.Now.Month.ToString() + "-" + DateTime.Now.Year.ToString() + ".xls";
    ...
    private void GrillaCargar(int PageIndex)
    {
        try
        {
            ...
            if ((Session["_perId"].ToString() == "10") | (Session["_perId"].ToString() == "9")) // Si es igual a familiar oculto edicion
            {
