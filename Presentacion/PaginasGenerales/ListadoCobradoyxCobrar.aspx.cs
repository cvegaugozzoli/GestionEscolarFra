...
MESSAGE:<br>" + oError.Message + "<br><br>EXCEPTION:<br>" + oError.InnerException + "<br><br>TRACE:<br>" + oError.StackTrace + "<br><br>TARGET:<br>" + oError.TargetSite +
    ...
    private void GrillaCargar2(int PageIndex)
    {
        try
        {
            ...
            Int32 CntId;
            CntId = Int32.Parse(ddlTipoConc.SelectedValue.ToString());
            ...
            if (Convert.ToInt32(NivelID.SelectedValue) != 100)
            {
                ...
                {
                    dt = ocnInscripcionConcepto.ObtenerTodoxCobrado(insId, cur, AnioCur, Convert.ToInt32(ddlDesde.SelectedValue), Convert.ToInt32(ddlHasta.SelectedValue), Convert.ToInt32(ddlConc.SelectedValue), car, CntId);//listado por curso, Estado = cursando
                }
                else
                {
...
MESSAGE:<br>" + oError.Message + "<br><br>EXCEPTION:<br>" + oError.InnerException + "<br><br>TRACE:<br>" + oError.StackTrace + "<br><br>TARGET:<br>" + oError.TargetSite +
    ...
    private void GrillaCargarTemp()
    {
        ...
        DateTime Hasta;
        Hasta = Convert.ToDateTime("01/01/1900");
        if (txtHasta.Text.Trim() != "")
        {
            Hasta = Convert.ToDateTime(txtHasta.Text.Trim());
        }
        ...
        if (Convert.ToInt32(NivelID.SelectedValue) != 100)
        {
            ...
            if (ddTipoListado.SelectedValue == "Cobrados")
            {
                ...
                {
                    dt = ocnInscripcionConcepto.ObtenerTodoxCobradoxBecados(insId, cur, AnioCur, Convert.ToInt32(ddlDesde.SelectedValue), Convert.ToInt32(ddlHasta.SelectedValue), Convert.ToInt32(ddlConc.SelectedValue), car, CntId);//listado por curso, Estado = cursando
                    // dt = ocnInscripcionConcepto.ObtenerTodoxVencidoExcel(insId, cur, AnioCur, Convert.ToInt32(ddlDesde.SelectedValue), Convert.ToInt32(ddlHasta.SelectedValue), Convert.ToInt32(ddlConc.SelectedValue), car, Hasta, CntId);  //listado por curso, Estado = cursando
                }
            }
            else
            {
                if (ddTipoListado.SelectedValue == "Por Cobrar")
                {
                    if (Convert.ToInt32(RbtSeleccion.SelectedValue) == 1)
                    {
                        dt = ocnInscripcionConcepto.ObtenerTodoxCobrarExcel(insId, cur, AnioCur, Convert.ToInt32(ddlDesde.SelectedValue), Convert.ToInt32(ddlHasta.SelectedValue), Convert.ToInt32(ddlConc.SelectedValue), car, Hasta, CntId);  //listado por curso, Estado = cursando
                    }
                    else
                    {
                        dt = ocnInscripcionConcepto.ObtenerTodoxCobrarExcelBecados(insId, cur, AnioCur, Convert.ToInt32(ddlDesde.SelectedValue), Convert.ToInt32(ddlHasta.SelectedValue), Convert.ToInt32(ddlConc.SelectedValue), car, Hasta, CntId);//listado por curso, Estado = cursando
                    }
                }
                else
                {
                    if (ddTipoListado.SelectedValue == "Vencidos")
                    {
                        if (Convert.ToInt32(RbtSeleccion.SelectedValue) == 1)
                        {
                            dt = ocnInscripcionConcepto.ObtenerTodoxVencidoExcel(insId, cur, AnioCur, Convert.ToInt32(ddlDesde.SelectedValue), Convert.ToInt32(ddlHasta.SelectedValue), Convert.ToInt32(ddlConc.SelectedValue), car, Hasta, CntId);  //listado por curso, Estado = cursando
                        ...
                        {
                            dt = ocnInscripcionConcepto.ObtenerTodoxVencidoExcelBecados(insId, cur, AnioCur, Convert.ToInt32(ddlDesde.SelectedValue), Convert.ToInt32(ddlHasta.SelectedValue), Convert.ToInt32(ddlConc.SelectedValue), car, Hasta, CntId);//listado por curso, Estado = cursando
                        }
                    }
                }
            }
            if (dt.Rows.Count != 0)
            {
                ...
                } else
                {
                    if (ddTipoListado.SelectedValue == "Por Cobrar")
                    {
                        FileName = "Informe por Cobrar " + DateTime.Now + ".xls";
                    } else
                    {
...
MESSAGE:<br>" + oError.Message + "<br><br>EXCEPTION:<br>" + oError.InnerException + "<br><br>TRACE:<br>" + oError.StackTrace + "<br><br>TARGET:<br>" + oError.TargetSite +
    ...
    protected void btnImprimir_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            String NomRep;
            Int32 curId = 0;
            if (curId2.SelectedValue.ToString() !="" )
            {
                curId = Int32.Parse(curId2.SelectedValue.ToString());
            }
            Int32 Anio = 0;
            int CuotaDesde = Int32.Parse(ddlDesde.SelectedValue.ToString());
            int CuotaHasta = Int32.Parse(ddlHasta.SelectedValue.ToString());
            int Concepto = 0;
            if (ddlConc.SelectedValue.ToString() != "")
            {
                Concepto = Int32.Parse(ddlConc.SelectedValue.ToString());
            }
            Int32 CarId;
            Int32 CntId;
            CntId = Int32.Parse(ddlTipoConc.SelectedValue.ToString());
            ...
            else
            {
                if (ddTipoListado.SelectedValue == "Por Cobrar")
                {
                    if (Convert.ToInt32(RbtSeleccion.SelectedValue) == 1)
                    {
                        NomRep = "ListadoPorCobrarNew.rpt";
                    }
                    else
                    {
                        NomRep = "ListadoPorCobrarNewBecados.rpt";
                    }
                } else
                {
                    if (Convert.ToInt32(RbtSeleccion.SelectedValue) == 1)
                    {
                        NomRep = "ListadoVencidosNew.rpt";
                    }
                    else
                    {
                        NomRep = "ListadoVencidosNewBecados.rpt";
                    }
                }
            ...
            if (ddTipoListado.SelectedValue == "Cobrados")
            {
                FuncionesUtiles.AbreVentana("Reporte.aspx?insId=" + insId + "&curId=" + curId + "&Anio=" + Anio + "&CuotaDesde=" + CuotaDesde + "&CuotaHasta=" + CuotaHasta + "&Concepto=" + Concepto + "&CarId=" + CarId + "&CntId=" + CntId + "&NomRep=" + NomRep);
            } else
            {
                ...
                FuncionesUtiles.AbreVentana("Reporte.aspx?insId=" + insId + "&curId=" + curId + "&Anio=" + Anio + "&CuotaDesde=" + CuotaDesde + "&CuotaHasta=" + CuotaHasta + "&Concepto=" + Concepto + "&CarId=" + CarId + "&hasta=" + hasta + "&CntId=" + CntId  + "&NomRep=" + NomRep);
            }
